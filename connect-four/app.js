document.addEventListener('DOMContentLoaded', () => {
    ...
    const displayCurretPlayer = document.querySelector('#current-player')
    let currentPlayer = 1;
    console.log('called')
    for(let i=0, len= squares.length;i< squares.length;i++) {
        ...
        squares[i].onclick = () => {
            if(squares[index+7].classList.contains('taken')) {
                if(currentPlayer === 1) {
                    ...
                    currentPlayer = 2;
                    displayCurretPlayer.innerHTML = currentPlayer
                } else {
                    squares[index].classList.add('taken')
